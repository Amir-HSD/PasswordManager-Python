...
def SaveUser(title,username,password,note):
    conn = db.connect()
    db.insert_user(conn,title,username,password,note)
...
def ShowAddNewForm(root):
    ...
    btn_save = tk.Button(add_user_window, text="Save", width=35,command=lambda: SaveUser(entry_title.get(),entry_username.get(),entry_password.get(),entry_note.get()))
